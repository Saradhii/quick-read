...
This Chrome extension, **Quick Read**, allows users to quickly summarize the textual content of the current web page they are viewing. With a click of a button, the extension extracts the main text from the page and uses the Gemini AI model to generate a short, bullet-point summary. Users can choose their desired summary length and enjoy a modern, styled interface.

## Features

*   **One-click AI Summarization:** Summarize the active tab's content using Google's Gemini model.
*   **Customizable Summary Length:** Choose between "Concise Summary" or "Detailed Summary" via a dropdown.
...
3.  **AI Summarization (`background.js`):**
    ...
    *   This request is sent to the Google Gemini API (model `gemini-1.5-flash-latest`).
