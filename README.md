...
This Chrome extension allows users to quickly summarize the textual content of the current web page they are viewing. With a click of a button, the extension extracts the main text from the page and **uses the Gemini AI model to generate** a short, bullet-point summary. Users can choose their desired summary length.

## Features

*   **One-click AI Summarization:** Summarize the active tab's content using Google's Gemini model.
*   **Customizable Summary Length:** Choose between "Short" (~2-3 points) or "Medium" (~4-5 points) summaries via a dropdown.
*   **Text Extraction:** Attempts to identify and extract the main textual content from a webpage.
*   **AI-Generated Summary:** Provides bullet points highlighting key sentences/topics from the text, generated by AI.
...
4.  In the popup that appears, **select your desired summary length (e.g., "Short (~2-3 points)" or "Medium (~4-5 points)") from the dropdown menu.**
5.  Click the "Summarize Page" button.
6.  A "Summarizing with AI..." message will appear. Please wait a few moments.
7.  The AI-generated summary will appear in the popup.
